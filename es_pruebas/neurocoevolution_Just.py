...
class Agent:
    ...
    def GetState(self, frame, agent=PLAYER_1_ID):
        ...
        frame_rgb[frame_rgb < 60] = 0
        frame_rgb[frame_rgb >= 60] = 255
    ...
    def evaluate(self, env, net, elite): #Siempre net(1) vs elite(2)
        ...
        while not done:
            frameCounter = {PLAYER_1_ID:1, PLAYER_2_ID:1}
            prev_action = {PLAYER_1_ID:0, PLAYER_2_ID:0}
            states = []
            for agent in self.env.agent_iter(5):
                ...
                frameCounter[agent]+=1
                if frameCounter[agent]==2:
                    frameCounter[agent]=0
                    real_state = state
                    repeat_action = False
                else:
                    repeat_action = True
                ...
                if self.env.terminations[agent] or self.env.terminations[negagent]:#abs(r[PLAYER_1_ID])>20 or abs(r[PLAYER_2_ID])>20:
                    ...
                    with torch.no_grad():
                        if repeat_action:
                            ...
                            action = redes[agent](real_state).max(1)[1].view(1, 1).item()#Hacer arreglo de redes
    ...
    def eval_vs_random(self, env, net):
        net.eval()
        self.reset()
        #state = self.reset()
        #state = self.get_state(env.state())
        ...
        while not done:
            ...
            prev_action = {PLAYER_1_ID:0, PLAYER_2_ID:0}
            states = []
            for agent in self.env.agent_iter(5000):
                negagent = (PLAYER_2_ID if agent == PLAYER_1_ID else PLAYER_1_ID)
                #_ , _ , done, info, _ = self.env.last()
                state, reward, done, info , _ = self.last(agent)
                frameCounter[agent]+=1
                ...
                if self.env.terminations[agent] or self.env.terminations[negagent]:#abs(r[PLAYER_1_ID])>20 or abs(r[PLAYER_2_ID])>20:
                    ...
                    with torch.no_grad():
                        if repeat_action:
                            ...
                            if(agent==PLAYER_1_ID):
                                action = net(real_state).max(1)[1].view(1, 1).item()#Hacer arreglo de redes
                                prev_action[agent] = action
                            else:
    ...
    def run(self):
        ...
        while True:
            ...
            for _ in range(MAX_BATCH_EPISODES):
                ...
                reward, steps = self.eval_with_noise(self.env, self.old_net, self.net, noise)
                batch_reward.append(reward)
                batch_steps += steps
                reward, steps = self.eval_with_noise(self.env, self.old_net, self.net, neg_noise)
                batch_reward.append(reward)
                batch_steps += steps
                steps2+=batch_steps
            ...
            #if str(step_idx)[-1:] == "0":
            reward_random.append(np.mean([self.eval_vs_random(self.env, self.net) for i in range(5)]))#print("Solved in %d steps" % step_idx)
                
                #print("REWARD vs RANDOM:", reward)
    ...
    def play(self):
        reward = []
        net = Net(self.state_size[0],self.action_size)
        net.load_state_dict(torch.load('net_ns05'))
        counter=0
        for p in net.parameters():
            for pd in p:
                counter +=1
            print(len(p.data))
        print(counter)
        reward.append(np.mean([self.eval_vs_random(self.env, net) for i in range(5)]))
...
if __name__ == "__main__":
    ...
    #net = agent.run()
    #fin = time.time()
    #torch.save(net.state_dict(), 'net_ns05_1')
